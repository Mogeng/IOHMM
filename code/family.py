...
class Poisson(Family):
    ...
    def deviance_weighted(self, endog, mu, w, scale=1.):
        ...
        '''
        endog_mu = self._clean(endog/mu)
        return 2*np.sum(w * endog*np.log(endog_mu))/scale
    ...
    def loglike_weighted(self, endog, mu, w, scale=1.):
        ...
        where gammaln is the log gamma function
        """
        return scale * np.sum(w*(-mu + endog*np.log(mu)-special.gammaln(endog+1)))
...
class Gaussian(Family):
    ...
    def deviance_weighted(self, endog, mu, w, scale=1.):
        ...
        `deviance` = sum((endog-mu)**2)
        """
        return np.sum(w * (endog-mu)**2)/scale
    ...
    def loglike_weighted(self, endog, mu, w, scale=1.):
        ...
        if isinstance(self.link, L.Power) and self.link.power == 1:
            ...
            assert sum_w > 0
            sigma = SSR / sum_w
            llf = - sum_w / 2 * np.log(2 * np.pi) - sum_w / 2 * np.log(sigma) - sum_w / 2
            return llf
...
class Gamma(Family):
    ...
    def deviance_weighted(self, endog, mu, w, scale=1.):
        ...
        """
        endog_mu = self._clean(endog/mu)
        return 2 * np.sum(w * ((endog - mu)/mu - np.log(endog_mu)))
...
class InverseGaussian(Family):
    ...
    def deviance_weighted(self, endog, mu, w, scale=1.):
        ...
        `deviance` = sum((endog=mu)**2/(endog*mu**2))
        """
        return np.sum(w * ((endog-mu)**2/(endog*mu**2)))/scale
