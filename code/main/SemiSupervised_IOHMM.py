...
class SemiSupervisedIOHMM:
    ...
    def setData(self, dfs_states):
        # states should be a list of dictionaries
        # [{0: np.array([0.3, 0.5, 0.2]), 3: np.array([0.3, 0.5, 0.2])},
        #  {1: np.array([0.3, 0.5, 0.2]), 4: np.array([0.3, 0.5, 0.2])},
        #  {2: np.array([0.3, 0.5, 0.2]), 5: np.array([0.3, 0.5, 0.2])}]
        # length of the states should be the same as len(dfs)
        # better to have all zeros or all ones
    ...
    def setInputs(self, covariates_initial, covariates_transition, covariates_emissions):
        ...
        self.inp_initials = [np.array(df[covariates_initial].iloc[0]).reshape(1,-1).astype('float64') for df, log_state in self.dfs_logStates]
        ...
        self.inp_transitions = [np.array(df[covariates_transition].iloc[1:]).astype('float64') for df, log_state in self.dfs_logStates]
...
if __name__ == "__main__":
    if len(sys.argv) != 1:
        print >> sys.stderr, "Usage: SemiSupervised_IOHMM<file>"
        exit(-1)
    ...
    SHMM = SemiSupervisedIOHMM(num_states=2, max_EM_iter=100, EM_tol=1e-4)
